...
{
	Super::BeginPlay();//no tiene porque tener nada, ni bien se inicializa el juego hace lo que esta en el begin play
...
void APieces::SpawnBlocks()//
{
    ...
    index;//con el numero almacenado genera la pieza de acuerdo del numero que se obtenga del index
    UE_LOG(LogTemp, Warning, TEXT("index=%d"), index);
    const std::vector<std::pair<float, float>>& YZs = Shapes[index];
    for (auto&& YZ : YZs)//funcion para que se mueva la pieza
    {
...
void APieces::Index()//
{
    index = FMath::RandRange(0, 6);//genera un numero aleatorio 
}
...
void APieces::Eliminar()// funcion para eliminar los bloques
{
    for (ABlock* B : Blocks)//ciclo for para agarrar el vector y bloque por bloque elimina 
    {
        B->Destroy();// destruye el bloque
        B = nullptr;// lo deja vacio
    }
}
